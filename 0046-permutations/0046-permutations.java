class Solution {
 ...
 all_permutations.add(arr);  return ; }  for (int index = 0; index < available.length; index++){ built.add(built.size(), available[index]); backTrack(built, removeAtIndex(index, available)); built.remove(built.size()-1); } } } 